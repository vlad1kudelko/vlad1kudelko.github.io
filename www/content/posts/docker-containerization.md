...
1. **Минимальный базовый образ** — `python:slim` или `alpine`.
2. **Не храните секреты в Dockerfile** — используйте переменные окружения и `.env` файлы.
3. **Проверяйте уязвимости** — `docker scan` или внешние сервисы.
4. **Обновляйте зависимости** — регулярно пересобирайте образы.
5. **Логи** — перенаправляйте stdout/stderr в систему логирования (ELK, Loki).
6. **Секреты** — используйте Docker secrets или внешние менеджеры (HashiCorp Vault, AWS Secrets Manager).
...
- **Prometheus** — экспортируйте метрики из приложения.
- **Grafana** — визуализируйте метрики.
- **Jaeger** — распределённый трейсинг.

## 10. Трудности и решения

- **Контейнер не запускается** — проверяйте логи, убедитесь, что порт открыт.
- **Ошибка зависимостей** — убедитесь, что `requirements.txt` актуален.
- **Большой размер образа** — используйте многоэтапную сборку, удаляйте лишние файлы.
- **Уязвимости в образе** — запустите `docker scan` и обновите базовый образ.
