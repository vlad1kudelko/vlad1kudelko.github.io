...
В исходном материале на Habr разбирается финальный этап такого стека: уже есть LLM на сервере, есть агентный backend на LangGraph, есть REST API, и дальше к нему подключается официальный интерфейс `agent-chat-ui` от LangChain. Для homelab и внутренних AI-инструментов это хороший пример того, как превратить эксперимент в сервис.
