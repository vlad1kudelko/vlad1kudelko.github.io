...
Андрей переносил legacy-монолит в микросервисы. При старте у каждого сервиса был фиксированный IP в конфиге. Когда один из сервисов переезжал на другой сервер, нужно было обновлять конфиги всех зависимых сервисов и рестартить их. Однажды при аварии сервис переехал автоматически на другой IP -- и полдня приложение работало с ошибками, пока кто-то не заметил устаревший адрес. Consul решил это: сервисы регистрируются сами, клиенты спрашивают Consul по DNS.
...
Команда Оли использовала Consul KV для feature flags. Разработчики включали новый алгоритм поиска через `consul kv put config/search/new_algo true` без деплоя. Watch в приложении подхватывал изменение за секунды. Если алгоритм давал неожиданные результаты -- одна команда возвращала старое значение. Функция feature flag работала без дополнительного сервиса.
