...
Роман руководил инфраструктурой для трёх команд. Каждая поднимала свой EKS кластер с нуля, копируя конфигурацию из старого проекта. Когда обнаружили CVE в конфигурации node groups, нужно было патчить в трёх местах -- и везде нашли разные версии одного кода. После создания общего модуля `terraform-modules/eks-cluster` все три команды ссылались на одну версию. Патч CVE: один PR, три обновления `?ref=v2.1.1`, и все кластеры защищены.
...
Света поддерживала инфраструктуру для пяти окружений. С монолитной конфигурацией `terraform apply` затрагивал всё сразу. После разделения на слои (networking → databases → applications) с отдельным state на каждый: networking меняется раз в квартал, приложения -- несколько раз в день. Каждая команда применяет свой слой независимо. Случайное изменение в applications не может затронуть networking.
