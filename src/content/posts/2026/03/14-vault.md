...
Павел работал в fintech-компании, где каждые три месяца происходила ротация паролей базы данных. Процесс занимал целый день: найти все места, где используется пароль, обновить, проверить что ничего не сломалось. При внедрении Vault Database Engine Павел настроил `default_ttl = 1h` для app-роли. Теперь каждый инстанс приложения каждый час получает новые учётные данные автоматически. Ротация стала бесконечной и бесплатной по усилиям.
...
Команда Ирины настроила External Secrets Operator + Vault вместо хранения секретов напрямую в Kubernetes Secrets. Обновление секрета -- одна команда в Vault CLI или веб-интерфейсе. External Secrets Operator синхронизирует изменение в Kubernetes Secret через `refreshInterval`. При следующем rolling update поды подхватывают новые значения. Ротация базы заняла 5 минут вместо дня ручной работы.
