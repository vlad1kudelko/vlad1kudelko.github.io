...
Сергей унаследовал Ansible-репозиторий с одним файлом `site.yml` на 800 строк. Разобраться, что и зачем делается, было невозможно. Когда обновление PostgreSQL ломало настройку nginx, никто не понимал, почему. Сергей потратил неделю на рефакторинг: разбил монолит на роли `postgresql`, `nginx`, `app`, `monitoring`. Каждая роль стала независимой. Теперь при обновлении PostgreSQL запускают только `--tags postgresql`, и в изменениях участвуют только нужные задачи.
...
Маша настраивала автоматическую установку Docker на 50 серверов. Нашла роль `geerlingguy.docker` в Galaxy -- 500+ stars, поддерживает Ubuntu, Debian, CentOS. Добавила в `requirements.yml`, запустила `ansible-galaxy role install -r requirements.yml`, написала плейбук из 10 строк. Docker установлен на всех серверах за 8 минут с правильной конфигурацией, которую проверили тысячи пользователей.
