...
Сергей получил задачу: новый разработчик должен запустить проект одной командой за 5 минут. Раньше это требовало установки 4 инструментов вручную и правки конфигов. Сергей добавил profiles в Compose: базовый `docker compose up -d` запускал приложение и базу, `--profile tools` добавлял pgAdmin и Redis Commander для отладки. В результирующий README вошла одна команда вместо страницы инструкций. Первый новый разработчик запустил всё за 8 минут, включая установку Docker.
...
Команда тестировала API через `docker compose up --abort-on-container-exit`. Тесты периодически падали с `connection refused` -- приложение стартовало до готовности базы. После добавления `condition: service_healthy` и правильного healthcheck тесты перестали флапать. CI-пайплайн стал стабильным, время на дебаг "случайных" падений сократилось с нескольких часов в неделю до нуля.
