...
Команда Артёма долго боролась с "drift" -- ручными изменениями в production, которые никто не помнил через неделю. Когда очередной хотфикс оказался не зафиксирован в Git, а следующий деплой затёр его, произошёл инцидент. После внедрения ArgoCD с `selfHeal: true` все изменения идут только через Git. Первую неделю разработчики привыкали к новому workflow и несколько раз злились, что их `kubectl patch` откатывается. Через месяц команда признала: непредвиденных расхождений в конфигурации prod больше нет.
...
Наташа настраивала деплой Django-приложения с автоматическими миграциями. Раньше миграции запускались как `initContainer`, что создавало проблемы: при откате приложения миграции уже применены. С ArgoCD PreSync hooks: миграции как отдельный Job в wave 0, приложение в wave 1. Если миграция падает -- приложение не обновляется, кластер остаётся в стабильном состоянии. Откат через `argocd app rollback` возвращает к предыдущей версии и без автоматических миграций.
