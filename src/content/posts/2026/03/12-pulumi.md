...
Паша был Python-разработчиком и ненавидел HCL. Terraform-конфигурации для его проекта писал DevOps-инженер, и каждое изменение требовало привлекать коллегу. После перехода на Pulumi с Python Паша самостоятельно написал инфраструктурный код за выходные: VPC, ECS кластер, RDS. Использовал Python dataclasses для конфигурации, pytest для валидации CIDR-диапазонов, list comprehension для создания subnets в каждой AZ. Для DevOps PR review стал занимать минуты -- код был понятен без объяснений.
...
Команда Игоря мигрировала с Terraform на Pulumi на Python. Самое большое удивление: написали pytest-тест, который проверяет что все S3 bucket имеют включённый versioning и шифрование, перед тем как ресурсы создаются. `pulumi preview` стал запускать валидацию конфигурации как часть CI. Ошибки конфигурации находили на этапе PR review, а не после деплоя в AWS.
