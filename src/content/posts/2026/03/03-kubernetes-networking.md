...
Виктор перенёс монолит на Kubernetes и потратил два дня, разбираясь, почему frontend не достучивается до backend. Оба сервиса работали, оба были healthy. Проблема оказалась в NetworkPolicy, которую установил security-инструмент автоматически: он закрыл весь трафик между namespace, и frontend не мог обратиться к backend в другом namespace. Одна строчка в NetworkPolicy с явным разрешением -- и сервисы заработали. Виктор потратил 5 минут на диагностику через `netshoot` и нашёл проблему, которую ручной поиск в конфигах мог занять день.
...
Ольга настраивала мониторинг нового namespace. Prometheus не мог скрапить метрики -- новый namespace был закрыт через `default-deny-all`. Вместо того чтобы открывать всё, Ольга добавила NetworkPolicy с `namespaceSelector: monitoring` для порта 9090. Метрики появились, остальной трафик остался закрытым. Аудит безопасности неделю спустя отметил это как пример правильного подхода: принцип минимальных привилегий даже для мониторинга.
