...
Дмитрий руководил небольшим стартапом, когда сервис неожиданно попал в TechCrunch. Трафик вырос с 50 до 5000 запросов в минуту за несколько часов. Сервер лёг не от нехватки CPU, а от исчерпания соединений PostgreSQL: приложение на Flask открывало новое соединение на каждый запрос, PostgreSQL упёрся в `max_connections = 100`. Дмитрий поднял PgBouncer за 20 минут, установил `default_pool_size = 20`, и сервис пережил хабраэффект без единого дополнительного сервера.
...
Анна настраивала Django-приложение с Celery для обработки очередей. После добавления PgBouncer в transaction mode задачи начали падать с `InternalError: prepared statement does not exist`. Django использует server-side cursors и prepared statements. Решение: отдельный PgBouncer pool в session mode специально для Celery workers, и transaction mode для HTTP-сервера. Два `[databases]` блока с разными портами, разные DSN в настройках Django -- задачи перестали падать, а основной сервер получил преимущества transaction pooling.
