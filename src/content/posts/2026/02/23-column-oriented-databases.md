...
Команда аналитики e-commerce работала с таблицей событий: 800 миллионов строк за 2 года. PostgreSQL справлялся, пока запросы были простыми. Когда нужен был retention-анализ -- "сколько пользователей из январской когорты вернулись через 7, 14, 30 дней" -- запрос занимал 4-6 минут. Дашборды обновлялись ночью, оперативно смотреть на данные было нельзя. После переезда на ClickHouse retention-запрос занял 1.8 секунды. Дашборды стали обновляться в реальном времени, команда продукта начала принимать решения по вчерашним данным, а не по недельным.
...
Максим строил аналитическую платформу для SaaS: 15 клиентов, у каждого своя схема событий. PostgreSQL держал транзакционные данные нормально, но аналитические запросы "сколько активных пользователей в день за последние 6 месяцев" становились медленными при росте. Вместо отдельного кластера ClickHouse он использовал DuckDB: Python-скрипт читал Parquet-файлы из S3 и отвечал за 0.3 секунды на данных объёмом 40 ГБ. Когда объём вырос до 500 ГБ, перешли на ClickHouse -- ровно то же SQL, другой движок.
