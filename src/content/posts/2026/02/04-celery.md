...
Celery — распределённая система очередей задач с тонкой настройкой производительности и надёжности. Большинство разработчиков не используют и половины возможностей — и обнаруживают это только при потере задач или утечках памяти в продакшене.
...
Воркер Celery — несколько вложенных уровней: главный процесс, пул рабочих процессов/потоков и prefetch-прокси для управления загрузкой очереди. Когда мы запускаем `celery -A myapp worker`, на самом деле создается следующая структура:
...
Конфигурация Celery определяет поведение при сбоях и производительность сильнее, чем код самих задач.
...
Мониторинг Celery — раннее предупреждение: длина очереди, время выполнения и error rate дают сигнал до того, как пользователи почувствуют проблему. Основные метрики, на которые нужно обращать внимание:
...
**Trade-off:** Celery проверен годами и покрывает большинство кейсов из коробки, но избыточен для простых задач. Arq или Huey легче, интеграций меньше.
