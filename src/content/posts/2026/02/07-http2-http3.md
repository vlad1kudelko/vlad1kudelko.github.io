...
HTTP/1.1 с 1997 года не менялся, и его узкие места хорошо известны: одно соединение — один запрос, заголовки не сжимаются, TCP-хендшейк при каждом подключении. HTTP/2 решил часть проблем через параллельные запросы в одном соединении. HTTP/3 пошёл дальше — заменил TCP на QUIC и встроил шифрование в транспортный слой.
...
Важно отметить, что QUIC — новый транспортный протокол: встраивает функции TCP и TLS в себя, работает поверх UDP.
...
HTTP/2 поддерживается везде и решает основные узкие места HTTP/1.1: параллельные запросы в одном соединении, сжатие заголовков, приоритизация потоков.
