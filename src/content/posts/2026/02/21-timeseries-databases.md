...
Женя администрировал мониторинг для IoT-платформы с 50 000 датчиков, каждый отправляет данные каждые 10 секунд. PostgreSQL справлялся первые полгода, но к 2 миллиардам строк запросы за неделю стали занимать по 45 секунд. Архитектор предложил ClickHouse, но DevOps-команда не хотела поддерживать новый сервис. Решение -- TimescaleDB: одна команда `CREATE EXTENSION`, одна команда `create_hypertable`, и те же запросы начали работать за 0.8 секунды. Миграция заняла выходные, PostgreSQL-знания остались применимы.
...
Команда аналитиков в финтех-компании строила дашборд для мониторинга транзакций в реальном времени. PostgreSQL с обычными таблицами не справлялся: запрос суммы транзакций по 5-минутным интервалам за последние сутки занимал 18 секунд при 500 млн строк. После перехода на TimescaleDB с continuous aggregate на `time_bucket('5 minutes', ...)` тот же запрос стал выполняться за 12 миллисекунд. Дашборд начал обновляться в реальном времени, аналитики перестали жаловаться на медленные отчёты.
