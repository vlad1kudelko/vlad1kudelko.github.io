...
Команда e-commerce добавляла систему рекомендаций. В PostgreSQL запрос "что купили пользователи, которые купили то же, что и ты" занимал 3-4 секунды на 5 млн пользователей и 50 млн покупок. Архитектор написал тот же запрос в Neo4j: одна строка Cypher, 120 миллисекунд. Для пользователей это разница между "рекомендации подгружаются незаметно" и "ждёшь результатов". После миграции только рекомендационного движка на Neo4j конверсия из категории "похожие товары" выросла на 18% -- просто потому что рекомендации стали появляться мгновенно.
...
Инженер по безопасности Андрей расследовал схему мошенничества. PostgreSQL-запрос для обнаружения цепочек транзакций через посреднические счета занимал 40 секунд. Граф с теми же данными в Neo4j: запрос `MATCH path = (fraud:Account)-[:TRANSFER*2..5]->(victim:Account)` -- 0.3 секунды. За одно расследование нашли 127 мошеннических аккаунтов, связанных через 3-4 уровня транзакций. Без граф-БД паттерн был бы виден только в Excel после часов аналитики.
