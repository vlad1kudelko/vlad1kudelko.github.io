...
Ansible устанавливается через пакетный менеджер или pip. Для работы требуется только Python на управляющей машине.
...
После установки проверьте версию: `ansible --version`.

## Базовая структура

Ansible использует несколько ключевых концепций: inventory (список хостов), playbooks (сценарии), modules (модули).

### Inventory

Inventory файл содержит список управляемых серверов, сгруппированных по назначению.
...
Handlers — это задачи, которые выполняются только при изменении состояния (notify). Они запускаются в конце playbook.
...
Для сложных сценариев разбивайте задачи на несколько playbook'ов:
