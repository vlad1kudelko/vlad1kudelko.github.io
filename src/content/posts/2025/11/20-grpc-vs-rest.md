...
| Формат | JSON, XML, текст | Protocol Buffers |
| Размер | Больше | Компактнее (3-10x) |
| Скорость | Медленнее | Быстрее (2-5x) |
| Типизация | Динамическая | Строгая (схема) |
| Кодогенерация | Нет (OpenAPI) | Да (из .proto) |
| Streaming | SSE, WebSocket | Нативный |
| Браузер | Нативная поддержка | Требуется grpc-web |
| Кэширование | HTTP кэш | Нет |
| Отладка | Легко (текст) | Сложнее (бинарный) |
| Документация | OpenAPI/Swagger | Protobuf + комментарии |
