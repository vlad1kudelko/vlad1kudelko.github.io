...
|-----|------------|--------|
| **Mock** | Проверка **как** используется | `assert_called_once_with()` |
| **Fake** | Рабочая реализация для тестов | Репозиторий в памяти |

**Mock** — лондонская школа TDD (проверка взаимодействия).

**Fake** — классическая школа TDD (проверка состояния).
