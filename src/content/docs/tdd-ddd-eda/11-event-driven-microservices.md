...
description: "Узнайте, как избежать циклической зависимости в микросервисах через Pub/Sub и Redis. Внедрите асинхронный обмен сообщениями для гибкой архитектуры. Читайте далее!"
pubDate: "2026-02-23"
order: 11
---
...
│ Тип                        │ Описание                                 │ Пример                 │
├────────────────────────────┼──────────────────────────────────────────┼────────────────────────┤
│ Согласованность исполнения │ Компоненты должны знать порядок операций │ RPC-вызовы             │
│ Согласованность времени    │ Компоненты должны работать одновременно  │ Синхронные HTTP-вызовы │
│ Согласованность имени      │ Компоненты согласуют только имя события  │ События ✅             │
└────────────────────────────┴──────────────────────────────────────────┴────────────────────────┘
```

`Альтернатива`: асинхронный обмен сообщениями
...
def test_change_batch_quantity_leading_to_reallocation():
    ...
    # 4. Ждем событие о переразмещении
...
├────────────┼────────────┼───────────────────────┤
│ Внутреннее │ OutOfStock │ Только внутри системы │
│ Внешнее    │ Allocated  │ Публикуется наружу    │
└────────────┴────────────┴───────────────────────┘
```

## Ключевые выводы Главы 11:

 1. Делите по глаголам, а не по существительным
