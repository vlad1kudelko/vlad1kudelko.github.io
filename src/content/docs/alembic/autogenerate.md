...
- Если параметр `EnvironmentContext.configure.include_schemas` установлен в значение `True`, все нестандартные «схемы», то есть имена, возвращаемые методом `get_schema_names()` класса Inspector. В документе SQLAlchemy «Указание имени схемы» подробно обсуждается понятие «схемы».
