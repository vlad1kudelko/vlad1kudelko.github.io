...
Давайте сделаем ещё одну, чтобы было с чем поиграться. Снова создадим файл ревизий:
...
Используя опцию `-r` в `alembic history`, мы также можем просматривать различные фрагменты истории. Аргумент `-r` принимает аргумент `[start]:[end]`, где может быть номер ревизии, символы, такие как `head`, `heads` или `base`, `current` для указания текущей(их) ревизии(й), а также отрицательные относительные диапазоны для `[start]` и положительные относительные диапазоны для `[end]`:
...
Просмотреть все изменения с 1975 года по `head`:
