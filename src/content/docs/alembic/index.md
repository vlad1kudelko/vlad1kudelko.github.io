...
Как упоминалось ранее, формат файла .ini в Alembic использует Python `ConfigParser` для анализа файла. В этой операции включена функция интерполяции ConfigParser для поддержки токена `%(here)s`, а также любых других токенов, настраиваемых пользователем через параметр `Config.config_args` при создании пользовательского объекта `Config`.
...
Файл содержит некоторую заголовочную информацию, идентификаторы текущей ревизии и «пониженной» ревизии, импорт базовых директив Alembic и пустые функции `upgrade()` и `downgrade()`. Наша задача — заполнить функции `upgrade()` и `downgrade()` директивами, которые внесут набор изменений в нашу базу данных. Обычно `upgrade()` требуется, а `downgrade()` — только если требуется возможность понижения ревизии, хотя, вероятно, это хорошая идея.
