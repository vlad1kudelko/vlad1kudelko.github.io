...
Функция select() принимает позиционные элементы, представляющие любое количество выражений типа «столбец» и/или «таблица», а также широкий спектр совместимых объектов, которые преобразуются в список SQL-выражений, из которых выполняется выборка, возвращаемая в качестве столбцов в результирующем наборе. Эти элементы также служат в более простых случаях для создания предложения FROM, которое выводится из переданных столбцов и табличных выражений:
...
Чтобы выполнить SELECT из отдельных столбцов с использованием подхода Core, доступ к объектам Column осуществляется из метода доступа Table.c, и их можно отправлять напрямую; предложение FROM будет выведено как набор всех объектов Table и других объектов FromClause, представленных этими столбцами:
...
В качестве альтернативы мы можем выбрать отдельные столбцы сущности ORM как отдельные элементы в строках результата, используя атрибуты, связанные с классом; когда они передаются в конструкцию, такую как select(), они преобразуются в Column или другое выражение SQL, представленное каждым атрибутом:
...
Имена меток, которые мы создаем, также могут быть упомянуты в предложении ORDER BY или GROUP BY оператора Select.
...
Мы можем использовать подобные выражения для генерации предложения WHERE, передавая полученные объекты методу Select.where():
...
Как упоминалось ранее, предложение FROM обычно выводится на основе выражений, которые мы устанавливаем в предложении columns, а также других элементов Select.

Если мы установим один столбец из определенной таблицы в предложении COLUMNS, то эта таблица также будет помещена в предложение FROM:
...
> При использовании Select.join_from() или Select.join() мы можем заметить, что предложение ON в соединении также выводится для нас в случаях простых внешних ключей. Подробнее об этом в следующем разделе.

Мы также можем явно добавлять элементы в предложение FROM, если это не выводится из предложения columns так, как нам нужно. Для этого мы используем метод Select.select_from(), как показано ниже: мы устанавливаем user_table первым элементом предложения FROM, а Select.join() — вторым элементом address_table:
...
Другой пример, когда может понадобиться Select.select_from(), — это когда в предложении columns недостаточно информации для предложения FROM. Например, для выполнения SELECT из общего выражения SQL count(*) мы используем элемент SQLAlchemy, известный как sqlalchemy.sql.expression.func, для создания функции SQL count():
...
> Установка самого левого предложения FROM в соединении — в руководстве по запросам ORM — содержит дополнительные примеры и примечания относительно взаимодействия Select.select_from() и Select.join().

## Указание ON

Предыдущие примеры использования JOIN иллюстрировали, что конструкция Select может объединять две таблицы и автоматически формировать предложение ON. В этих примерах это происходит, поскольку объекты таблиц user_table и address_table содержат единое определение ForeignKeyConstraint, которое используется для формирования этого предложения ON.

Если левая и правая цели соединения не имеют такого ограничения или имеется несколько ограничений, необходимо напрямую указать предложение ON. Как Select.join(), так и Select.join_from() принимают дополнительный аргумент для предложения ON, который задаётся с использованием той же механики SQL-выражений, что и в предложении WHERE:
...
Оператор SQL SELECT включает предложение ORDER BY, которое используется для возврата выбранных строк в заданном порядке.

Предложение GROUP BY построено аналогично предложению ORDER BY и предназначено для разделения выбранных строк на группы, к которым могут быть применены агрегатные функции. Предложение HAVING обычно используется с предложением GROUP BY и имеет форму, аналогичную предложению WHERE, за исключением того, что оно применяется к агрегатным функциям, используемым внутри групп.

## ORDER BY

Предложение ORDER BY строится на основе конструкций SQL-выражений, обычно основанных на объектах Column или подобных. Метод Select.order_by() принимает одно или несколько из этих выражений позиционно:
...
При использовании агрегатных функций в SQL предложение GROUP BY играет ключевую роль, поскольку позволяет разбивать строки на группы, к каждой из которых агрегатные функции будут применяться индивидуально. При запросе неагрегированных столбцов в предложении COLUMNS оператора SELECT SQL требует, чтобы все эти столбцы были подчинены предложению GROUP BY, напрямую или косвенно, на основе первичного ключа. Предложение HAVING в этом случае используется аналогично предложению WHERE, за исключением того, что оно отфильтровывает строки на основе агрегированных значений, а не непосредственного содержимого строк.

SQLAlchemy реализует эти два предложения с помощью методов Select.group_by() и Select.having(). Ниже мы иллюстрируем выборку полей имени пользователя, а также количества адресов для пользователей, имеющих более одного адреса:
...
Важным приёмом, особенно в некоторых бэкендах баз данных, является возможность применять ORDER BY или GROUP BY к выражению, уже указанному в предложении columns, без повторного указания выражения в предложении ORDER BY или GROUP BY, а вместо этого использовать имя столбца или имя с меткой из предложения COLUMNS. Эта форма доступна путём передачи строкового текста имени методу Select.order_by() или Select.group_by(). Переданный текст не отображается напрямую; вместо этого отображается имя, присвоенное выражению в предложении columns, которое отображается как имя этого выражения в контексте, вызывая ошибку, если совпадение не найдено. Унарные модификаторы asc() и desc() также могут использоваться в этой форме:
...
Теперь, когда мы выбираем данные из нескольких таблиц и используем объединения, мы быстро сталкиваемся с необходимостью многократно ссылаться на одну и ту же таблицу в предложении FROM оператора. Это достигается с помощью псевдонимов SQL — синтаксиса, предоставляющего альтернативное имя таблице или подзапросу, из которого на них можно ссылаться в операторе.
